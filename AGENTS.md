...
- `uv run --script scripts/vectorize-svg-text.py --fonts <dir> <in.svg> <out.svg>` — turns `<text>` into `<path>`. Run **by hand, rarely**: only when the wording, size or position of the text in `assets/brand/*-source.svg` or `.github/assets/repository-card-source.svg` changes. It needs Inter and Plus Jakarta Sans, which CI deliberately does not have — `assets/brand/FONT-NOTICE.md` records where to fetch them, their SHA-256, and why the OFL permits publishing the outlines. It refuses to emit anything if shaping yields `.notdef`, so a conversion that lost the French diacritics cannot be committed. Five files are outlined; two are build inputs rasterised to PNG, three are **served as SVG** and additionally owe a textual alternative and `currentColor`, both gated by `scripts/test-assets.py`. Regenerating anything under `assets/brand/` requires re-running `./scripts/generate-assets.sh` so `manifest.json` carries the new SHA-256.
